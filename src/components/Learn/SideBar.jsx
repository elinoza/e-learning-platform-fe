...
class SideBar extends Component {
  ...
  completed = (array, index) => {
    ...
    if (array) {
      completed =
        this.props.player.currentCourseProgress &&
  ...
  render() {
    ...
    return (
      <>
        <div
          className={
            showSideBar ? "sidebar showSideBar" : " sidebar closedSideBar"
          }
        >
          <a
            className="closebtn"
          ...
          <div className="sidebar-toggle-wrapper ">
            <Button
              className="sidebar-toggle-Button  pt-3 w-100 d-flex align-items-start"
              onClick={() => this.props.sideBarToggle(false)}
              size="sm"
            >
              {" "}
          ...
          <ul className="m-0 p-0">
            {currentCourse.playList &&
              currentCourse.playList.map((item, index) => (
                <li
                  className={
                    activeIndex === index ? "activeContent d-flex" : "d-flex"
                  }
                  onClick={() => this.postProgress(currentCourse._id, index)}
                >
                  {!this.completed(currentCourseProgress.completed, index) ? (
                    <IoMdRadioButtonOff className="icons mr-2 mt-3" />
                  ) : (
                    <FcCheckmark className="icons mr-2 mt-3" />
                  )}
                  <div>
                    <p className="m-0">{item.contentName}</p>
                    <p
                      className="m-0"
                      style={{ color: "#b7b0b0", fontSize: "14px" }}
                    >
                      {" "}
                      {this.formatSeconds(item.duration)}
                    </p>
                  </div>{" "}
                  <BsBookmark
                    style={{ fontSize: "20px" }}
                    className="icons  ml-auto mt-2"
                  />
                </li>
              ))}
          </ul>
